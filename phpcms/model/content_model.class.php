...
class content_model extends model {
	...
	public function add_content($data,$isimport = 0) {
		...
		//如果发布时间比现在晚，则说明这是条定时发布的任务，status 改为1 ，并且写入定时任务表中。
		if($systeminfo['inputtime'] > SYS_TIME){
			$systeminfo['status'] = $data['status'] = 1 ;
		}else{
			$systeminfo['status'] = $data['status'] = 99 ;
		}
		...
		//如果inputtime 时间大于现在时间，说明是定时任务。则写入dingshi表。 
		// 王官庆  3.4 
		if($systeminfo['inputtime'] > SYS_TIME){
			insert_dingshi($modelid,$systeminfo['catid'],$id,$systeminfo['inputtime']);
		}
		...
		if($id && isset($_POST['othor_catid']) && is_array($_POST['othor_catid'])) {
			...
			foreach ($_POST['othor_catid'] as $cid=>$_v) {
				...
				if($modelid==$mid) {
					...
					//如果inputtime 时间大于现在时间，说明是定时任务。则写入dingshi表(同时发布到其它栏目也是如此，使用新的ID，及modelid)
					// 王官庆 3.4 
					if($systeminfo['inputtime'] > SYS_TIME){
						insert_dingshi($mid,$systeminfo['catid'],$newid,$systeminfo['inputtime']);
					}
