...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			// 王官庆 3.4 修订定时文章在后台的列表中显示。
			if($steps){
				$is_gongzhuoliu = 1;
			}else{
				$is_gongzhuoliu = 0;//正常进入管理列表页，不是一审二审
			}
			$status = $steps; 			

			if($status && $status!==0){//点击的一 二审核按钮
				$where = 'catid='.$catid.' AND status='.$status;
			}else{
				$where = 'catid='.$catid;  //打开发布的定时文章   王官庆 3.4 
			}
			//搜索
	...
	public function add() {
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				//如果是视频或爆款类型，分析视频URL，更新 platform_vid platform 2个字段（这样不需要人工填写）
				if(($modelid==11 or $modelid==12) && $_POST['info']['play_url']!=''){
					$video_array = get_videoinfo_by_playurl($_POST['info']['play_url']);
					if(!empty($video_array)){
						//分析到视频信息，并且不为空，则更新入库
						$this->db->set_model($modelid);
						$this->db->update(array("platform_vid"=>$video_array['platform_vid'],"platform"=>$video_array['platform']),array("id"=>$id));
					}
				}
	...
	* 更新插入的文章
	*/
	public function chang_add_article(){
		$id = intval($_POST['id']);
		$title = safe_replace($_POST['title']);
		$thumb = safe_replace($_POST['thumb']);
