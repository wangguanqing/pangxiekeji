...
<tbody>
	...
	?>
		...
		}?><span<?php echo title_style($r['style'])?>><?php echo $r['title'];?></span></a> <?php if($r['thumb']!='') {echo '<img src="'.IMG_PATH.'icon/small_img.gif" title="'.L('thumb').'">'; } if($r['posids']) {echo '<img src="'.IMG_PATH.'icon/small_elite.gif" title="'.L('elite').'">';} if($r['islink']) {echo ' <img src="'.IMG_PATH.'icon/link.png" title="'.L('islink_url').'">';}?>
		<!-- //打分等级 以及所属类型 -->
		&nbsp;&nbsp;
		<?php if($r['grade']){ ?>
			<font color="red"><?php echo $r['grade'];?></font>分
		<?php } ?>
		&nbsp;&nbsp;
		<?php if($r['typeid']){ ?>
			<font color="red"><?php echo get_typename($r['typeid']);?></font>
		<?php } ?>
		<!-- 是否定时任务 -->
		<?php  
			if($r['status']!=99 ){
		?>
				<?php if($r['inputtime']> SYS_TIME){ ?>
				&nbsp;&nbsp;
				<img src="<?php echo IMG_PATH;?>clock.png" width="20" height="20"> <?php  echo date("Y-m-d H:i:s",$r['inputtime']);?>

				<?php echo "<font color=red>等待上线中...</font>";}?>
		<?php 		
			}
		?>
