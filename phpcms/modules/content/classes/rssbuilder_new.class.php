...
class RSSBuilder extends RSSBase {
	...
	*/
	function RSSBuilder($encoding = '',
		...
		$this->setImageTitle($image_title);
	...
	*/
	function setImageTitle($image_title = '') {
		if (!isset($this->image_title) && strlen(trim($image_title)) > 0) {
			$this->image_title = (string) trim($image_title);
		} // end if
	} // end function
	...
	*/
	function setImageUrl($image_url = '') {
		if (!isset($this->image_url) && strlen(trim($image_url)) > 0) {
			$this->image_url = (string) trim($image_url);
		} // end if
	} // end function
	...
	*/
	function createOutputV091() {
		...
		if (strlen($this->image_link) > 0) {
			...
			$this->output .= (string) '<title>' . $this->image_title . '</title>' . "\n";
			$this->output .= (string) '<url>' . $this->image_url . '</url>' . "\n";
			$this->output .= (string) '<link>' . $this->image_link . '</link>' . "\n";
			if (strlen($this->description) > 0) {
				$this->output .= (string) '<description>' . $this->description . '</description>' . "\n";
			} // end if
			$this->output .= (string) '</image>' . "\n";
	...
	*/
	function createOutputV200() {
		...
		if (strlen($this->date) > 0) {
			$this->output .= (string) '<pubDate><![CDATA[' .$this->date . ']]> </pubDate>' . "\n";
			$this->output .= (string) '<lastBuildDate>' .$this->date . '</lastBuildDate>' . "\n";
		} // end if
		...
		if (strlen($this->about) > 0) {
			$this->output .= (string) '<link><![CDATA[' . $this->about . ']]> </link>' . "\n";
		} // end if

		if (strlen($this->title) > 0) {
			$this->output .= (string) '<title><![CDATA[' . $this->title . ']]></title>' . "\n";
		} // end if
		...
		if (count($this->getItemList()) > 0) {
			foreach ($this->getItemList() as $id) {
				...
				if (strlen($item->getTitle()) > 0 && strlen($item->getLink()) > 0) {
					...
					if ($this->use_dc_data === TRUE && strlen($item->getDate()) > 0) {
						$this->output .= (string) '<pubDate><![CDATA[' . $item->getDate() . ']]> </pubDate>' . "\n";
					} // end if
					...
					if (strlen($item->getAuthor()) > 0) {
						$this->output .= (string) '<source><![CDATA[' . $item->getAuthor() . ']]> </source>' . "\n";
					} // end if
