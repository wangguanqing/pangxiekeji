...
class index {
	...
	public function init() {
		...
		if(isMobile() || $_GET['wap']==1){
			include template('wap','index','v4');
		}else{
	...
	public function show() {
		...
		//移动端判断
		if($_GET['comefrom']=='toutiao'){
			include template('wap','show_toutiao');exit;
		}
		if(isMobile() || $_GET['wap']==1){
			include template('wap','show');exit;
		} 
		//pc端调PC的模版
	...
	/**
	 * 按typeid进行列表显示
	 * 参数：typeid, catid,modelid,type(最新，最热) 
	 *
	 */
	...
	public function type_list() {
		...
		//SEO 信息
		$SEO = seo(1, '', $type_array[0]['name'].' - 螃蟹科技', '螃蟹科技是一家智能设备新媒体平台，致力于为读者带来详实且有价值的信息，并为智能设备领域的创业者及参与人士提供多方位服务。','螃蟹科技,硬件评测,VR');
		...
		//移动端判断 
		if(isMobile() || $_GET['wap']==1){
			include template('wap','type_list');exit;
		}else{
			include template('content','type_list'); 	
		}
	 }
	 ...
	 //WAP所有视频
	 public function type_modelid(){
	 	$modelid = intval($_GET['modelid']);
	 	$this->db->set_model($modelid);
	 	$sql = array("status"=>99);
	 	$return = $this->db->listinfo($sql, $order = 'id desc', $page = 1, $pagesize = 9);
	 	//SEO
		$SEO = array("title"=>'螃蟹科技',"keywords"=>'螃蟹科技,硬件评测,VR',"description"=>'螃蟹科技是一家智能设备新媒体平台，致力于为读者带来详实且有价值的信息，并为智能设备领域的创业者及参与人士提供多方位服务。');
